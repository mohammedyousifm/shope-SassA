<div class="bg-white rounded-2xl shadow p-6 mb-8">
    ...
    <form action="{{ route('clients.profile.update') }}" method="post">
        ...
        <button type="submit"
            class="prevent-double-click mt-6 w-full bg-gradient-to-r from-teal-500 to-green-500 text-white py-2 rounded-lg text-sm font-semibold hover:opacity-90">
            حفظ التعديلات
        </button>
    </form>
</div>
...
<div class="bg-white rounded-2xl shadow p-6">
    ...
    <div class="space-y-4 text-sm">
        <form action="{{ route('clients.profile.password') }}" method="post">
            @csrf
            @method('patch')
            <div>
    ...
    <button
        class="prevent-double-click mt-6 w-full bg-gradient-to-r from-yellow-400 to-orange-500 text-white py-2 rounded-lg text-sm font-semibold hover:opacity-90">
        تحديث كلمة المرور
    </button>
    </form>
</div>